...
namespace PostProcess
{
    void Invert(const ColorBuffer& colorBuffer)
    {
        ...
        for (int i = 0; i < (colorBuffer.width * colorBuffer.height); i++)
        {
            ...
            color.r = 255 - color.r; //<to invert the color component, subtract the color component from 255>
            color.g = 255 - color.g; //<to invert the color component, subtract the color component from 255>
            color.b = 255 - color.b; //<to invert the color component, subtract the color component from 255>
        }
    }
        ...
        for (int i = 0; i < colorBuffer.width * colorBuffer.height; i++) {
            ...
            for (int iy = -1; iy <= 1; iy++)
            {
                for (int ix = -1; ix <= 1; ix++)
                {
                    ...
                    v += ((color_t*)source.data)[(x + ix) + (y + iy) * source.width].r * kv[ix + 1][iy + 1];
                }
            }
