...
glm::vec3 Scene::Trace(const ray_t& r, float tMin, float tMax, raycastHit_t& hit, int depth)
{
    ...
    if (rayHit)
    {
        ...
        //•	If the material is the Emissive material then emissive will be that color, if not emissive will be black(no emissive)
            //o	Remember in the material base class, black { 0, 0, 0 } is returnedand only the Emissive material returns a color
        glm::vec3 emissive = hit.material->Emitter(hit.uv, hit.point);

        if (!hit.material->Scatter(r, hit, attenuation, scattered))
        {
            //•	If the Scatter function returns false, then the emissive color will be returned and the ray tracing will be done for that ray
                return emissive;
        }
