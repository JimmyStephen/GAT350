...
{
    if (x < 0 || x >= colorBuffer.width || y < 0 || y >= colorBuffer.height) {
        return;
    }
    uint8_t alpha = color.a;
    uint8_t invAlpha = 255 - color.a;
...
int Framebuffer::Lerp(int a, int b, float t) { return (int)(a + ((b - a) * t)); }
...
void Framebuffer::DrawImage(int x1, int y1, Image* image)
{
    for (int y = 0; y < image->colorBuffer.height; y++)
    {
        ...
        {
            int sx = x1 + x;
            if (sx < 0 || sy < 0 || sx > colorBuffer.width || sy > colorBuffer.height) continue;
